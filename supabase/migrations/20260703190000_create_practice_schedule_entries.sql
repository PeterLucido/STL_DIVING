...
drop policy if exists "Authenticated users can delete their practice schedule entries" on public.practice_schedule_entries;
create policy "Authenticated users can delete their practice schedule entries"
on public.practice_schedule_entries
for delete
to authenticated
using (auth.jwt() ->> 'email' = email);
